...
class ShanghaiTechDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for i in range(left_lim, right_lim, self.stride):
            ...
            if self.use_resnet:
                frame_img_resnet = self.transform_resnet(frame_img_orig)
                frame_batch_resnet.append(frame_img_resnet)
            frame_img = self.transform(frame_img_orig)
            frame_img_normalized = self.normalize(frame_img)
            frame_batch.append(frame_img_normalized)
            if self.need_eval:
                frame_batch_eval.append(frame_img)

        frame_batch = torch.stack(frame_batch).permute((1, 0, 2, 3))
        returnable = {'img': frame_batch, 'index': index}
        if self.need_eval:
            frame_batch_eval = torch.stack(frame_batch_eval).permute((1, 0, 2, 3))
            returnable['img_eval'] = frame_batch_eval
        if self.use_resnet:
            frame_batch_resnet = torch.stack(frame_batch_resnet).permute((1, 0, 2, 3))
            returnable['img_resnet'] = frame_batch_resnet
        else:
            returnable['img_resnet'] = None
        return returnable
