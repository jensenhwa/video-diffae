...
for T in x_values:
    ...
    for i in range(NUM_GPUS):
        ...
        with open(f"semantic_optical_flow_scores_{i}_{T}.log", "r") as fp:
            for line in fp:
    ...
    with open("/home/eprakash/diffae/idx_to_centerframe_16.txt", "r") as fp:
        for line in fp:
    ...
    anomaly_mean_scores.append(np.mean(anomaly_scores))
    normal_mean_scores.append(np.mean(normal_scores))
...
with open("data.csv", "w+", newline="") as f:
    writer = csv.writer(f)
    writer.writerows([anomaly_mean_scores, normal_mean_scores, aurocs, auprcs])
