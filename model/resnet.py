...
class ResNetEncoderModel(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        out_flow = self.flow_model(torch.flatten(flow, start_dim=0, end_dim=1))
        out_flow = torch.flatten(torch.unflatten(out_flow, 0, (batch_size, 9)), start_dim=1)
        out_flow = self.downsize_flow(out_flow)
        print(out.shape, out_flow.shape)
        assert False
        return out  # (N, 512)
